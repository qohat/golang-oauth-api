...
func Test_SendEmail_ValidEmail(t *testing.T) {
	userApp.AskForResetPasswordFn = func(*entity.User) (*entity.User, map[string]string) {
		return &entity.User{
			ID:        1,
			FirstName: "victor",
			LastName:  "steven",
			Email:     "qohatpp@gmail.com",
		}, nil
	}

	r := gin.Default()
	r.POST("/auth/verification", s.AskForResetPassword)
	inputJSON := `{
		"first_name": "victor",
		"last_name": "steven",
		"email": "steven@example.com",
	...
	req, err := http.NewRequest(http.MethodPost, "/auth/verification", bytes.NewBufferString(inputJSON))
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	rr := httptest.NewRecorder()
